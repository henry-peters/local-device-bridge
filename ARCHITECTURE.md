...
- `internal/core`: canonical TV/display, console, and computer metadata, capabilities, commands, manager, pairing interfaces, and persistence interfaces.
- `internal/discovery`: route-aware local-host identification, per-interface SSDP and Bonjour discovery, bounded direct probes for documented TV/computer services, console identity discovery, and ARP fallback.
...
- `internal/console`: vendor-neutral console inventory/status/Wake-on-LAN adapter. It does not impersonate console accounts or claim universal power-off support.
...
Inventory visibility is controlled by the TVs/displays, consoles, computers, and offline-device settings. Mobile, audio, and anonymous Other records are not exposed. Dashboard Settings writes the shared live state and config file; CLI, Telegram, and agents read the same manager list.

Discovery records may be duplicated across providers. The manager normalizes them into TVs & displays, Game consoles, or Computers; computer cards retain only supported labels such as macOS, Windows, or Raspberry Pi. Generic Linux hosts, phones, speakers, and anonymous ARP peers stay internal and are not shown because an IP address alone is not a safe device identity. Stable DUID/MAC identity wins over provider IDs, and richer vendor metadata wins over generic service records. Persisted adapters are recreated on startup so a sleeping supported TV, console, or Mac can still be targeted for Wake-on-LAN when a MAC is known.
...
Recognized LG/Sony TVs, Windows computers, Raspberry Pi systems, and consoles may be listed with only the capabilities their adapters can prove. Their detail page gives honest access guidance. Generic Linux computers, anonymous ARP peers, phones, and speakers remain hidden. Adding a brand requires a real authenticated adapter and tests rather than sending Samsung keys to unrelated hardware.
